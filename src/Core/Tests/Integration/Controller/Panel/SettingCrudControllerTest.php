...
use App\Core\Controller\Panel\SettingCrudController;
...
class SettingCrudControllerTest extends BaseTestCase
{
    public function testAccessDeniedForNonAdminUser(): void
    {
        ...
        $this->client->request('GET', '/panel?crudControllerFqcn=' . urlencode(SettingCrudController::class) . '&crudAction=new');
    ...
    public function testCreateSetting(): void
    {
        ...
        $crawler = $this->client->request('GET', '/panel?crudControllerFqcn=' . urlencode(SettingCrudController::class) . '&crudAction=new');
        ...
        $this->assertResponseRedirects('/panel?crudAction=index&crudControllerFqcn=' . urlencode(SettingCrudController::class));
    ...
    public function testEditSetting(): void
    {
        ...
        $crawler = $this->client->request('GET', '/panel?crudAction=edit&crudControllerFqcn=' . urlencode(SettingCrudController::class) . '&entityId=' . $setting->getId());
        $form = $crawler->selectButton('Save setting')->form([
            'Setting[value]' => 'Updated Value',
        ]);

        $this->client->submit($form);
        $this->assertResponseRedirects('/panel?crudAction=index&crudControllerFqcn=' . urlencode(SettingCrudController::class) . '&entityId=' . $setting->getId());
