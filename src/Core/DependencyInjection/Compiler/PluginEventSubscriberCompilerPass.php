...
/**
 * Registers plugin event subscribers during container compilation.
 */
class PluginEventSubscriberCompilerPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        ...
        foreach ($enabledPlugins as $pluginData) {
            // Check if plugin has 'eda' capability
            if (!in_array('eda', $pluginData['manifest']['capabilities'] ?? [], true)) {
                continue;
            }

            $this->registerEventSubscribers($container, $pluginData, $projectDir);
        }
    }

    private function registerEventSubscribers(ContainerBuilder $container, array $pluginData, string $projectDir): void
    {
        $pluginName = $pluginData['name'];
        $pluginPath = $projectDir . '/plugins/' . $pluginName;
        $subscriberPath = $pluginPath . '/src/EventSubscriber';

        // Check if EventSubscriber directory exists
        ...
        try {
            // Find all PHP files in EventSubscriber directory
            $iterator = new \RecursiveIteratorIterator(
                new \RecursiveDirectoryIterator($subscriberPath, \RecursiveDirectoryIterator::SKIP_DOTS)
            );
            ...
            foreach ($iterator as $file) {
                ...
                $relativePath = str_replace($subscriberPath . '/', '', $file->getPathname());
                $className = $pluginNamespace . '\\EventSubscriber\\' . str_replace(['/', '.php'], ['\\', ''], $relativePath);

                // Verify class exists and implements EventSubscriberInterface
                if (!class_exists($className)) {
                    error_log("Event subscriber class not found: {$className}");
                    continue;
                }

                if (!in_array(EventSubscriberInterface::class, class_implements($className) ?: [], true)) {
                    error_log("Class {$className} does not implement EventSubscriberInterface");
                    continue;
                }

                // Register as service with event subscriber tag
            ...
            error_log("Failed to register event subscribers for plugin {$pluginName}: {$e->getMessage()}");
        }
    }
